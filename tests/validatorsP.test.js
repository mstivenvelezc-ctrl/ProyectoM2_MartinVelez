...
// Pruebas para validarPublished
describe('validarPublished', () => {
  test('acepta published válido (true)', () => {
    expect(validarPublished(true)).toBe(null);
    });
    test('acepta published válido (false)', () => {
    expect(validarPublished(false)).toBe(null);
    });
    test('acepta published undefined (campo opcional)', () => {
    expect(validarPublished(undefined)).toBe(null);
    });
    test('rechaza published con valor no booleano', () => {
    expect(validarPublished('sí')).toContain('debe ser un booleano');
    });
});
